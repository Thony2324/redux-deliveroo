...
import { useIsEnterprise } from "../../contexts/EnterpriseContext";

const Header = ({ resto_name, resto_description, resto_picture }) => {
  const enterpriseCtx = useIsEnterprise();

  console.log(enterpriseCtx);
        ...
        <div className="RestaurantInfos--center">
          <div className="RestaurantInfos--texts">
            ...
            <label>
              <input
                type="checkbox"
                checked={enterpriseCtx.isEnterprise}
                onChange={e => enterpriseCtx.setIsEnterprise(e.target.checked)}
              />
              <span>Je suis une entreprise</span>
            </label>
            <p>{resto_description}</p>
            <div className="reduc">
              <span className="reduc-svg">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
              ...
              <div className="reduc-desc">
                <p>
                  - {DISCOUNT * 100} % sur tout le menu pour toute commande de
                  plus de 50,00 € (hors frais de livraison).
                </p>
              </div>
            </div>
          </div>
